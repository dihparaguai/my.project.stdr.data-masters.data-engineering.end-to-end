...
## Tema:
Desenvolvimento de pipeline de engenharia de dados voltado ao setor bancário, usando dados de ações provenientes do Yahoo Finance, com foco em ingestão e estruturação de dados financeiros, para simular flixos em batch e em streaming.
...
## Fonte de dados:
- Yahoo Finance: em formato DataFrame do Pandas.
...
2. **Ingestão de Dados**
   ...
   - Os dados vão direto para a camada Raw(Local).
3. **Processamento de Dados**
   - Os dados seguem pelas camadas: Raw(Local) → Bronze(MinIO) → Silver(MinIO) → Gold(MinIO).
...
![Imagem da Arquitetura](./docs/architecture_diagram.png)
